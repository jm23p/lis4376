# LIS 4376: AI Applications

## Joshua Mann

### Project 1 Requirements:

1. Download .txt files for p1.ipynb to pull from
2. Follow helper video to format/code p1.ipynb
3. Utilize data for predictive analyses
4. Complete P1 skillsets
5. Upload screenshots/gifs of p1.ipynb and 3 skillsets

### README.md file should include the following items:

* Screenshots/Gifs of Skillsets
* Gif of P1
* Links to P1 Github and Skillset directories

## P1 Gif:

![P1 Gif](p1.gif)
---
## P1Files and Links:

[P1 .ipynb File](p1.ipynb)

## P1Skillsets:

### Skillset 7 (Pseudo-Random Numbers List):
![SS7 Screenshot](<Screenshot 2026-03-01 133418.png>)

### Skillset 8 (Interest Calculator):
![SS8 Screenshot](<Screenshot 2026-03-01 143957.png>)

### Skillset 9 (Guessing Game):
![SS9 Screenshot](<Screenshot 2026-03-02 215010.png>)